...
def expose_alloc(prog: Program) -> Program:
    ...
    def ea_stmt(s: Stmt) -> List[Stmt]:
        match s:
            case Assign(x, Prim('tuple', args)):
                ...
                # TODO
                tag = mk_tag(args)
                all_stmts += [Assign(tmp1_vars, Prim('allocate', [Constant(bytes_needed), Constant(tag)]))]
    ...
    def ea_stmts(stmts: List[Stmt]) -> List[Stmt]:
        return [stmt for s in stmts for stmt in ea_stmt(s)]
...
def explicate_control(prog: Program) -> ctup.CProgram:
    ...
    #print(ctup.CProgram(basic_blocks))
...
def select_instructions(prog: ctup.CProgram) -> x86.X86Program:
    ...
    def si_stmt(stmt: ctup.Stmt) -> List[x86.Instr]:
        match stmt:
            # TODO add cases for tuple subscipt, tuple_set, allocate, collect
            ...
            case ctup.Assign(x, ctup.Prim('tuple_set', [atm1, atm2, atm3])):
                return [x86.NamedInstr('movq', [si_atm(atm1), x86.Reg('r11')]),
                        x86.NamedInstr('movq', [si_atm(atm3), x86.Deref('r11', atm3.val*8)])]
            case ctup.Assign(x, ctup.Prim('collect', [atm1])):
                return [x86.NamedInstr('movq', [x86.Reg('r15'), x86.Reg('rdi')]),
                        x86.NamedInstr('movq', [si_atm(atm1), x86.Reg('rsi')]),
                        x86.Callq('collect')]
